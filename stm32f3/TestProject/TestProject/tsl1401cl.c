...
void TSL_GPIO_Configuration(void)
{
	...
	GPIO_InitStructure.GPIO_Pin   = TSL_CLOCK_PIN | TSL_SERIAL_PIN | GPIO_Pin_13;
...
void TSL_MeasurePixels(int16_t *pixels)
{
	uint16_t i;
	/* Cycle through the values very quickly to reduce integration time */
	TSL_StartOutputCycle();
	for (i = 0; i < TSL_PIXEL_COUNT+1; i++) TSL_ClockPulse();
	DelayUs(TSL_CHARGE_TRANSFER_TIME_US);

	/* Integration for the next output cycle begins on the 19th clock pulse */
	TSL_StartOutputCycle();
	...
	TSL_ClockPulse();
	/* Minimum allowed delay before next output cycle */
	//DelayUs(TSL_CHARGE_TRANSFER_TIME_US); 
}
