...
#define Z_ADJUST_BUTTON_UP_PIN     14 // PJ1
#define Z_ADJUST_BUTTON_DOWN_PIN   15 // PJ0
...
void setup()
{
  ...
  digitalWrite(STAT_LED_BLUE, LOW); // turn it off
    ...
    /* Pin init for retract/extrude buttons */
    SET_INPUT(RETRACT_BUTTON_FORWARD_PIN);
    SET_INPUT(RETRACT_BUTTON_REVERSE_PIN);
    WRITE(RETRACT_BUTTON_FORWARD_PIN, 1); /* Pull up */
    WRITE(RETRACT_BUTTON_REVERSE_PIN, 1); /* Pull up */

    /* Pin init for z-adjust up/down buttons */
    SET_INPUT(Z_ADJUST_BUTTON_UP_PIN);
    SET_INPUT(Z_ADJUST_BUTTON_DOWN_PIN);
    WRITE(Z_ADJUST_BUTTON_UP_PIN, 1);     /* Pull up */
    WRITE(Z_ADJUST_BUTTON_DOWN_PIN, 1);   /* Pull up */
...
void e0_adjust_step(const bool direction)
{
    cli();
    enable_e0();
    uint8_t old_e0_dir_pin = READ(E0_DIR_PIN);
    ...
    /* Precise 1 microsecond delay (might not be needed) */
    __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t");  /* 250 ns delay with 16Mhz chips */
    __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t");
    __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t");
    __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t");
    WRITE(E0_STEP_PIN, INVERT_E_STEP_PIN);
    ...
    WRITE(E0_DIR_PIN, old_e0_dir_pin);
    sei();
}

void e1_adjust_step(const bool direction)
{
    cli();
    enable_e1();
    uint8_t old_e1_dir_pin = READ(E1_DIR_PIN);
    ...
    //get old pin state back.
    WRITE(E1_DIR_PIN, old_e1_dir_pin);
    sei();
}

void z_adjust_step(const bool direction)
{
    cli();      /* Disable interrupts since this function should never be interrupted! */
    enable_z(); /* Ensure z-motor driver is turned on */

    /* Remember current direction so it can be restored later*/
    uint8_t old_z_dir_pin = READ(Z_DIR_PIN);

    /* Execute a single z-axis step */
    ...
    __asm__("nop\n\t""nop\n\t""nop\n\t""nop\n\t");
    WRITE(Z_STEP_PIN, INVERT_Z_STEP_PIN);
    WRITE(Z_DIR_PIN, old_z_dir_pin); /* Restore previous direction */

    /* Increment or decrement z-axis home offset by a single step */
    if (direction) add_homing[Z_AXIS] -= (1.0 / axis_steps_per_unit[2]);
    else           add_homing[Z_AXIS] += (1.0 / axis_steps_per_unit[2]);
    sei(); /* re-enable interrupts */
}

const int STEP_DELAY_MIN_US = 75;
const int STEP_DELAY_MAX_US = STEP_DELAY_MIN_US * 100;

void retract_button_update()
{
    int extruder_select = READ(EXTRUDER_SELECT_PIN);
    if (!digitalRead(RETRACT_BUTTON_FORWARD_PIN))
    {
        delay(20); /* Debounce delay */
        if (!digitalRead(RETRACT_BUTTON_FORWARD_PIN))
        {
            uint32_t time_elasped_us = 0;
            int32_t offset_delay_us = STEP_DELAY_MAX_US;

            while (!digitalRead(RETRACT_BUTTON_FORWARD_PIN))
            {
                /* Keep track of how long the user is pressing the button */
                time_elasped_us += offset_delay_us + STEP_DELAY_MIN_US;

                /* Increase the feedrate over time (acceleration) */
                if ((time_elasped_us > 1000) && offset_delay_us > 0)
                {
                    time_elasped_us = 0;
                    offset_delay_us = (offset_delay_us > STEP_DELAY_MIN_US) ? (offset_delay_us-STEP_DELAY_MIN_US) : 0;
                }

                if (extruder_select) e0_adjust_step(true); else e1_adjust_step(true);
                delayMicroseconds(STEP_DELAY_MIN_US + offset_delay_us);
            }
        }
    }
    else if (!digitalRead(RETRACT_BUTTON_REVERSE_PIN))
    {
        delay(20); /* Debounce delay */
        if (!digitalRead(RETRACT_BUTTON_REVERSE_PIN))
        {
            uint32_t time_elasped_us = 0;
            int32_t offset_delay_us = STEP_DELAY_MAX_US;
           
            while (!digitalRead(RETRACT_BUTTON_REVERSE_PIN))
            {
                /* Keep track of how long the user is pressing the button */
                time_elasped_us += offset_delay_us + STEP_DELAY_MIN_US;
                ...
                if (extruder_select) e0_adjust_step(false); else e1_adjust_step(false);
                delayMicroseconds(STEP_DELAY_MIN_US + offset_delay_us);
            }
        }
    }
    ...
    else if (!digitalRead(RETRACT_BUTTON_REVERSE_PIN))
    {
        ...
        if (!digitalRead(RETRACT_BUTTON_REVERSE_PIN))
        {
            retract_button_reverse();
        }
    }*/
}

void z_adjust_button_update()
{
    if (!digitalRead(Z_ADJUST_BUTTON_UP_PIN))
    {
        delay(20); /* Debounce delay */
        if (!digitalRead(Z_ADJUST_BUTTON_UP_PIN))
        {
            /* Continue to adjust z-axis while button is held down */
            while (!digitalRead(Z_ADJUST_BUTTON_UP_PIN))
            {
                z_adjust_step(true);
                delayMicroseconds(2000); /* reduce delay = faster z-adjust, increase delay = slower z-adjust */
            }
        }
    }
    else if (!digitalRead(Z_ADJUST_BUTTON_DOWN_PIN))
    {
        delay(20); /* Debounce delay */
        if (!digitalRead(Z_ADJUST_BUTTON_DOWN_PIN))
        {
            /* Continue to adjust z-axis while button is held down */
            while (!digitalRead(Z_ADJUST_BUTTON_DOWN_PIN))
            {
                z_adjust_step(false);
                delayMicroseconds(2000); /* reduce delay = faster z-adjust, increase delay = slower z-adjust */
            }
